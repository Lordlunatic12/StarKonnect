...
const renderTags=(role,level,languages=[],tools=[],isSearch=!1)=>{return[role,level,...languages,...tools].map(v=>{return `<span class="tag tag-lg tag-light-green">${v} ${isSearch ? ' <img src="images/icon-remove.svg" class="remove-icon" alt="remove" /> ' : ''}</span>`})};
const renderSearchTags=(data,isSearch=!0)=>{console.log(data);return data.map(v=>{return `<span class="tag tag-lg tag-light-green">${v} ${isSearch ? ' <img src="images/icon-remove.svg" class="remove-icon" alt="remove" /> ' : ''}</span>`})}
let data=[{"id":1,
			"company":"Photosnap",
	...
	if(searchItems.length>0)
	{
		...
		await jobSearchTags.forEach(jobSearchTag=>{
			jobSearchTag.addEventListener('click',async(e)=>{
				e.preventDefault();
				let item=e.target.parentNode.innerText;searchItems=searchItems.filter(searchItem=>searchItem!==item);
				let filteredData=[];
				if(searchItems.length>0)
